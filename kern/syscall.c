...
static int check_addr_scale(unsigned int val, unsigned int min,
			    unsigned int max) {
...
#ifdef DEBUG_SYSCALL_C
    cprintf
	("===[0x%x]Execute in sys_env_set_status===env_id:0x%x,status:0x%x\n",
...
static int sys_page_map(envid_t srcenvid /*In fork: child */ , void *srcva,
			envid_t dstenvid /*In fork: parent */ ,
			void *dstva, int perm) {
    ...
    cprintf("===[0x%x]Execute in sys_page_map,perm:0x%x\n", curenv->env_id,
	    perm);
    cprintf
	("   srcenvid:0x%04x,srcva:0x%08x --> dstenvid:0x%04x, dstva:0x%08x ===\n",
...
//            || (perm & (~PTE_SYSCALL)))
	...
	cprintf
	    ("!!! Error: (!(src_page = page_lookup(src_e->env_pgdir,srcva,&src_pte))) !!!\n");
	return -E_INVAL;
    }
    // Perm has the same restrictions as in sys_page_alloc, except
    // that it also must not grant write access to a read-only
	...
	cprintf
	    ("!!! Error: (!((*src_pte) & PTE_W)) && (perm & PTE_W) !!!\n");
	return -E_INVAL;
    }
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva,
	...
	if ((!((*src_pte_store) & (PTE_W))) && (perm & PTE_W)) {
	    cprintf
		("!!! sys_try_send: srcva's pte_store's permission error !!!\n");
	    return -E_INVAL;
	}
    }
	...
	if (NULL !=
	    page_lookup(uenv->env_pgdir, uenv->env_ipc_dstva, NULL)) {
	    cprintf("!!! sys_try_send: dstva's page doesn't exist !!!\n");
	    return -E_NO_MEM;
	}

	sys_page_map(curenv->env_id, srcva, envid, uenv->env_ipc_dstva,
		     perm);

	if (NULL ==
	    page_lookup(uenv->env_pgdir, uenv->env_ipc_dstva,
			&dst_pte_store)) {
...
#endif
    // LAB 4: Your code here.
    if (IPC_NO_PAGE != dstva
	&& (dstva >= (void *) UTOP || (((int) dstva) % PGSIZE))) {
