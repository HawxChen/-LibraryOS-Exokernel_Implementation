...
void trap_init_percpu(void) {
	...
	thiscpu->cpu_ts.ts_esp0 =
	    KSTACKTOP - (KSTKSIZE + KSTKGAP) * thiscpu->cpu_id;
...
static void trap_dispatch(struct Trapframe *tf) {
	...
	&& tf->tf_trapno <= IRQ_OFFSET_HANDLE_MAX) {
...
void page_fault_handler(struct Trapframe *tf) {
    ...
    cprintf("[%08x] user fault va %08x ip %08x\n", curenv->env_id,
	    fault_va, tf->tf_eip);
    cprintf
	("[0x%08x] user fault va 0x%08x,tf->err 0x%03x ,tf->eip 0x%08x, tf->esp 0x%08x\n",
    ...
    user_mem_assert(curenv, curenv->env_pgfault_upcall, PGSIZE,
		    PTE_U | PTE_P);
#ifdef DEBUG_TRAP_C
