...
static void pgfault(struct UTrapframe *utf) {
    ...
    //Look Jos.out
    if ((r =
	 sys_page_alloc(0, (void *) PFTEMP, PTE_P | PTE_U | PTE_W)) < 0) {
	r = -3;
	goto FAIL;
    }
    memmove((void *) PFTEMP, (void *) ROUNDDOWN(addr, PGSIZE), PGSIZE);
    if ((r =
	 sys_page_map(0, (void *) PFTEMP, 0,
...
static int duppage(envid_t envid, unsigned pn) {
	...
	sys_page_map(0,
		     (void *) PGNUM2LA(pn),
		     envid, (void *) PGNUM2LA(pn), PGOFF(vpt[pn]));
...
envid_t fork(void) {
	...
	if (0 != vpd[dir_i]) {
	    cprintf
		("addr:0x%08x,dir_i = 0x%x,vpd[dir_i]=0x%08x,&vpd[dir_i]:0x%08x\n",
		 dir_i << PDXSHIFT, dir_i, vpd[dir_i], &vpd[dir_i]);
	    for (table_i = dir_i * 1024; table_i < ((dir_i + 1) * 1024);
		 table_i++) {
		//                cprintf("\t\ttable_i:0x%08x\n",table_i);
		if (0 != vpt[table_i]) {
		    cprintf
			("\taddr:0x%08x, pageNumber = 0x%x, table index = 0x%x, vpt[table_i]=0x%08x, &vpt[table_i]=0x%08x\n",
			 (table_i << PTXSHIFT), table_i, table_i & 0x3FF,
			 vpt[table_i], &vpt[table_i]);
		}
	    }
	}
    }
	...
	if (pgn == PGNUM(UXSTACKBASE)) {
	    //allocation
	    sys_page_alloc(child_envid, (void *) UXSTACKBASE,
			   PTE_U | PTE_W | PTE_P);
	} else if (pgn < PGNUM(UXSTACKBASE)
		   && ((PTE_P | PTE_U) ==
		       (vpd[pgn >> 10] & (PTE_P | PTE_U)))
