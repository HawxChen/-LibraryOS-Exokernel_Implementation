...
static int init_stack(envid_t child, const char **argv,
		      uintptr_t * init_esp);
...
init_stack(envid_t child, const char **argv, uintptr_t * init_esp) {
    ...
    // a null pointer.
    argv_store =
	(uintptr_t *) (ROUNDDOWN(string_store, 4) - 4 * (argc + 1));
    ...
    *init_esp = UTEMP2USTACK(&argv_store[-2]);
    ...
    if ((r =
	 sys_page_map(0, UTEMP, child, (void *) (USTACKTOP - PGSIZE),
		      PTE_P | PTE_U | PTE_W)) < 0)
	goto error;
...
map_segment(envid_t child, uintptr_t va, size_t memsz,
	...
	if (i >= filesz) {
		...
		return r;
	    if ((r =
		 sys_page_map(0, UTEMP, child, (void *) (va + i),
			      perm)) < 0)
